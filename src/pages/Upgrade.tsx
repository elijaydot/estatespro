  ...
  return <div className="min-h-screen bg-muted/30 px-4 py-8 md:px-8">
    <div className="mx-auto max-w-7xl space-y-8">
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">{query.data?.plans.map((plan) => {
        ...
        return <Card key={plan.id} className="flex flex-col rounded-md"><CardHeader><CardTitle>{plan.name}</CardTitle><div><span className="text-3xl font-semibold">${((usd?.amount_minor || 0) / 100).toFixed(0)}</span><span className="text-muted-foreground"> / month</span></div></CardHeader><CardContent className="flex-1 space-y-3">{features.map((feature: string) => <div key={feature} className="flex gap-2 text-sm"><Check className="mt-0.5 h-4 w-4 shrink-0" /><span className="capitalize">{feature}</span></div>)}</CardContent><CardFooter><Button className="w-full" asChild><a href={`mailto:sales@fishgatepro.com?subject=${encodeURIComponent(`Upgrade to ${plan.name}`)}`}>Talk to us about {plan.name}</a></Button></CardFooter></Card>;
      })}</div>
