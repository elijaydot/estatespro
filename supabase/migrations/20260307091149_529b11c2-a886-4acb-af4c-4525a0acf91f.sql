...
-- Convert ALL RLS policies from RESTRICTIVE to PERMISSIVE (default)
-- PostgreSQL doesn't allow ALTER POLICY for this, so we DROP and recreate
...
CREATE POLICY "PMs view assigned payments" ON public.payments FOR SELECT TO authenticated
...
CREATE POLICY "Tenants send messages" ON public.messages FOR INSERT TO authenticated
  WITH CHECK (EXISTS (SELECT 1 FROM (SELECT get_tenant_id_by_user(auth.uid()) AS tid) x WHERE x.tid::text = messages.sender_id::text));

CREATE POLICY "Tenants view their messages" ON public.messages FOR SELECT TO authenticated
  USING (EXISTS (SELECT 1 FROM (SELECT get_tenant_id_by_user(auth.uid()) AS tid) x WHERE x.tid::text = messages.sender_id::text OR x.tid::text = messages.recipient_id::text));
...
CREATE POLICY "Tenants view recurring bills" ON public.recurring_bills FOR SELECT TO authenticated
  USING (
    ...
    OR (tenant_id IS NULL AND property_id IS NULL AND user_id IN (SELECT t.user_id FROM tenants t WHERE t.tenant_user_id = auth.uid()))
  );
