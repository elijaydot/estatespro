// MFA Disable: requires a valid TOTP or recovery code; removes secret + recovery codes.
...
serve(async (req) => {
  ...
  const rateCheck = checkRateLimit(req, { keyPrefix: "mfa-disable", limit: 10, windowMs: 60_000 });
  ...
  try {
    ...
    let ok = false;
    if (isRecovery) {
      const { data } = await userClient.rpc("consume_recovery_code", { p_code: rawCode.toUpperCase() });
      ok = data === true;
    } else {
    ...
    await service.from("user_mfa").delete().eq("user_id", user.id);
    await service.from("security_recovery_codes").delete().eq("user_id", user.id);
    await userClient.rpc("log_security_event", { p_event_type: "mfa_disabled", p_metadata: {} });
